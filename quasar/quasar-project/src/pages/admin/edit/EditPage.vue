  ...
  <q-page class="festival-page">
    ...
    <div class="hero-header">
      ...
      <div class="hero-content">
        ...
        <h1 class="hero-title">แก้ไขเทศกาล</h1>
        <p class="hero-sub">แก้ไขเทศกาลสุดพิเศษให้ผู้คนได้อวยพรกัน</p>
      </div>
    </div>

    <!-- ===== FORM ===== -->
    <q-form ref="formRef" @submit.prevent="submitAdd" class="form-body">
      <div class="form-container">
          ...
          <transition-group name="wish-list" tag="div" class="wish-list-wrapper">
            <div v-for="(wish, i) in wishWordList" :key="wish + i" class="wish-item">
              ...
              <div class="wish-actions">
                <button class="wish-btn wish-btn--edit" @click="editWish(i)">
                  <q-icon name="edit" size="16px" />
                </button>
                <button class="wish-btn wish-btn--delete" @click="deleteWish(i)">
        ...
        <div class="fest-card animate-in" style="animation-delay: 0.19s">
          ...
          <div class="card-header-row">
            <div class="card-header-info">
              <span class="card-header-count">{{
                cardFileList.length + existingCards.length
              }}</span>
          ...
          <div v-if="existingCards.length > 0 || cardFileList.length > 0" class="card-grid">
            <!-- การ์ดที่โหลดมาจาก server -->
            <div v-for="(card, i) in existingCards" :key="'existing-' + i" class="card-thumb">
              <q-img :src="card.previewUrl" ratio="1" class="card-thumb-img" />
              <button class="card-thumb-remove" @click="removeExistingCard(i)">
                <q-icon name="close" size="14px" />
              </button>
              <div class="card-thumb-badge">เดิม</div>
            </div>

            <!-- การ์ดใหม่ที่ user เพิ่ม -->
            <div v-for="(file, i) in cardFileList" :key="'new-' + i" class="card-thumb">
              ...
              <button class="card-thumb-remove" @click="removeCard(i)">
                <q-icon name="close" size="14px" />
              </button>
              <div class="card-thumb-badge card-thumb-badge--new">ใหม่</div>
            </div>
          </div>
        ...
        <div class="animate-in" style="animation-delay: 0.25s">
          <button type="submit" class="submit-btn" :class="{ loading }" @click.prevent="submitAdd">
    ...
    <q-dialog v-model="AddWishDialog" :maximized="$q.screen.lt.sm">
      <div class="custom-dialog" :class="{ 'custom-dialog--mobile': $q.screen.lt.sm }">
        <div class="custom-dialog-header">
          ...
          <button class="dialog-close" @click="AddWishDialog = false">
    ...
    <q-dialog v-model="EditWishDialog" :maximized="$q.screen.lt.sm">
      <div class="custom-dialog" :class="{ 'custom-dialog--mobile': $q.screen.lt.sm }">
        <div class="custom-dialog-header">
          ...
          <button class="dialog-close" @click="EditWishDialog = false">
            <q-icon name="close" size="18px" />
          </button>
        </div>
    ...
    <q-dialog v-model="showCardDialog" :maximized="$q.screen.lt.sm">
      <div class="custom-dialog" :class="{ 'custom-dialog--mobile': $q.screen.lt.sm }">
        <div class="custom-dialog-header">
          ...
          <button class="dialog-close" @click="showCardDialog = false">
            <q-icon name="close" size="18px" />
          </button>
        </div>
        <div class="custom-dialog-body">
...
const imageFile = ref<File | null>(null); // รูปใหม่ที่ user เลือก
const existingImageUrl = ref<string>(''); // blob url ของรูปเดิมจาก server
const existingImageName = ref<string>(''); // ชื่อไฟล์เดิม (ส่งกลับ API ถ้าไม่เปลี่ยน)
...
const cardFileList = ref<File[]>([]); // การ์ดใหม่ที่ user เพิ่ม

interface ExistingCard {
  imageName: string; // ชื่อไฟล์เดิม (ส่งกลับ API)
  previewUrl: string; // blob url สำหรับแสดงผล
}
const existingCards = ref<ExistingCard[]>([]); // การ์ดเดิมจาก server
...
/* ===== ON MOUNTED: โหลดข้อมูล ===== */
// onMounted(() => {
//   // รับ id จาก route params หรือ query
//   const id = (route.params.id as string) || (route.query.id as string)
//   if (id) {
//     await fetchFestivalId(id)
//   }
// })
...
const fetchFestivalId = async (id: string) => {
  ...
  try {
    ...
    // คำอวยพร
    // wishWordList.value = data.wisher?.map((w: any) => w.wishWord) || []
    wishWordList.value = data.wisher?.map((w: Wisher) => w.wishWord) || [];

    // การ์ดอวยพร: โหลด blob url ของแต่ละการ์ด
    if (data.card && data.card.length > 0) {
      const cardPromises = data.card.map(async (c: Card) => {
        const previewUrl = await getImageUrl(c.imageCard);
        return { imageName: c.imageCard, previewUrl };
      });
...
const submitAdd = async () => {
  ...
  // valid ถ้ามี imageFile ใหม่ หรือมีรูปเดิมจาก server อยู่แล้ว
  const isImageValid = !!imageFile.value || !!existingImageName.value;

  if (!isNameValid || !isImageValid) {
    $q.notify({
      color: 'negative',
      message: 'กรุณากรอกชื่อเทศกาลและอัปโหลดรูป',
      icon: 'warning',
    });
    return;
  }
  ...
  loading.value = true;
  try {
    let festivalImageName = existingImageName.value; // ใช้ชื่อเดิมก่อน

    // ถ้า user เลือกรูปใหม่ → อัปโหลด
    if (imageFile.value) {
    ...
    // อัปโหลดการ์ดใหม่ที่ user เพิ่ม
    ...
    // รวมการ์ดเดิมที่ยังไม่ได้ลบ + การ์ดใหม่
    const allCardImageNames = [
      ...existingCards.value.map((c) => c.imageName),
      ...newCardImageNames,
    ];
    ...
    // ถ้าเป็น edit ให้ใช้ PUT พร้อม festivalId, ถ้าเป็น create ให้ใช้ POST
    let response;
    if (festivalId) {
      response = await api.put(`/admin/festival/${festivalId}`, payload, {
    ...
    $q.notify({
      color: 'positive',
      message: response.data.message || 'บันทึกสำเร็จ',
      icon: 'celebration',
      position: 'top',
      timeout: 2500,
    });
