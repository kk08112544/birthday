  ...
  <q-page class="login-page">
    ...
    <q-dialog v-model="authDialog">
      <div class="auth-dialog">
        ...
        <!-- :key forces animation re-mount on each new dialog open -->
        <div
...
const onSubmit = async () => {
  ...
  try {
    const response = await api.post<LoginResponse>('/backoffice/auth/login', {
      userName: userName.value.trim(),
      password: password.value,
    });

    const { uId, firstName, userName: uName, role } = response.data.user;
...
// Auto-closing progress bar — shrinks to zero over the dialog's display duration
