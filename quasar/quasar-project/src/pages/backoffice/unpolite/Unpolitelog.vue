            ...
            <q-td :props="props">
              <div class="actor-wrap">
                <div class="actor-avatar-outer">
                  <div class="actor-avatar" :class="`actor-avatar--${getAvatarColor(props.row.actor)}`">
    ...
    <q-dialog v-model="viewDialog" :maximized="$q.screen.xs">
      <div class="log-view-dialog" :class="{ 'log-view-dialog--mobile': $q.screen.xs }">
        ...
        <div class="lvd-body" v-if="viewRow">
          ...
          <!-- Word -->
          <div class="lvd-section">
            <div class="lvd-label">คำไม่พึงประสงค์</div>
            <div class="lvd-word-chip">
              <q-icon name="label_off" size="16px" color="negative" class="q-mr-xs" style="flex-shrink:0" />
              {{ viewRow.word }}
            </div>
          </div>
          ...
          <div class="lvd-section" v-if="viewRow.action === 'UPDATE'">
            ...
            <div v-if="viewRow.diffs.length > 0" class="lvd-diff-wrap">
              <div v-for="d in viewRow.diffs" :key="d.key" class="lvd-diff-row">
...
// Avatar colour palette — cycle by first char code
...
function getDiffs(e: LogEntry): { key: string; old: string; new: string }[] {
...
.hero-inner {
  ...
  gap: 14px; flex-wrap: wrap; max-width: 960px; margin: 0 auto;
}
.hero-icon-wrap {
...
.hero-back-btn {
  ...
  font-family: 'Noto Sans Thai', sans-serif !important; font-weight: 600 !important; letter-spacing: 0 !important;
...
// ─── Content ──────────────────────────────────────────────────────────────────
.content-wrap { max-width: 960px; margin: -1.75rem auto 0; padding: 0 1rem 3rem; display: flex; flex-direction: column; gap: 1rem; }
...
.stat-chip {
  ...
  box-shadow: 0 2px 12px rgba(55,48,163,0.08); border: 1px solid rgba(79,70,229,0.12);
...
.filter-pill {
  ...
  &--all    { background: #f5f3ff; color: $purple; border-color: rgba(91,33,182,0.3);
              &.active,&:hover { background: $purple; color: #fff; border-color: $purple; } }
...
.table-card {
  ...
  box-shadow: 0 4px 28px rgba(55,48,163,0.09); border: 1px solid rgba(79,70,229,0.08);
}
.styled-table {
  :deep(thead tr th) {
    ...
    border-bottom: 2px solid rgba(124,58,237,0.12); white-space: nowrap;
  }
  :deep(tbody tr) { transition: background 0.15s; &:hover { background: rgba(124,58,237,0.03) !important; } }
  :deep(tbody tr td) {
    font-size: clamp(0.78rem,2vw,0.875rem); color: $text-main;
    border-bottom: 1px solid rgba(124,58,237,0.06); vertical-align: middle;
  }
  :deep(tbody tr td:first-child) { color: $text-muted; font-size: 0.75rem; text-align: center; }
  :deep(.q-table__bottom) { border-top: 1px solid rgba(124,58,237,0.08); background: $surface-2; font-size: 0.82rem; }
}
...
// ─── Word Badge ───────────────────────────────────────────────────────────────
.word-badge {
  display: inline-flex; align-items: center; background: $red-soft; color: $red;
  border-radius: 8px; padding: 4px 10px; font-weight: 600; font-size: 0.85rem;
...
// ─── Diff ─────────────────────────────────────────────────────────────────────
.diff-row { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; font-size: 0.8rem; }
.diff-old { background: $red-soft; color: #b91c1c; border-radius: 6px; padding: 2px 8px; font-weight: 600; word-break: break-all; }
.diff-new { background: $green-soft; color: #15803d; border-radius: 6px; padding: 2px 8px; font-weight: 600; word-break: break-all; }
...
// Each palette: [gradient-from, gradient-to, text, ring]
$avatar-palettes: (
...
.actor-username-chip {
  ...
  color: $purple-mid; width: fit-content;
...
.dialog-drag-handle {
  ...
  background: $surface;
  &::before { content: ''; width: 36px; height: 3px; border-radius: 2px; background: rgba(124,58,237,0.2); }
}
...
.lvd-close-btn {
  ...
  background: rgba(91,33,182,0.07); color: $text-muted;
